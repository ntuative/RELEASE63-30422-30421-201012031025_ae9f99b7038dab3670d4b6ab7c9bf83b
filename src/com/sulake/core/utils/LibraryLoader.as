package com.sulake.core.utils
{
   ...
   public class LibraryLoader extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1511:uint = 1;
      
      protected static const const_1512:uint = 2;
      
      protected static const const_1513:uint = 4;
      
      protected static const const_1510:uint = 3;
      ...
      protected var var_252:URLRequest;
      
      protected var var_739:XML;
      
      protected var var_610:Class;
      ...
      protected var var_2594:String = "";
      
      protected var var_2595:String = "";
      
      protected var var_1870:int;
      
      protected var var_1158:int;
      
      protected var var_1159:int;
      
      protected var var_2671:uint = 0;
      
      protected var var_2596:Boolean = true;
      ...
      {
         return !!this.var_252 ? this.var_252.url : null;
      }
      ...
      {
         return !!this.var_928 ? uint(this.var_1158 - this.var_1870) : uint(getTimer() - this.var_1870);
      }
      ...
      override public function dispose() : void
      {
         if(!disposed)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DISPOSE,this.var_349,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
            try
            {
               this.var_38.close();
            }
      ...
      public function load(param1:URLRequest, param2:int = 5) : void
      {
         ...
         this._name = parseNameFromUrl(this.var_252.url);
      ...
      {
         if(this.var_927 && !_disposed)
         {
            this.var_927 = false;
            if(!this.var_928 && this.var_252)
            {
               this.var_1158 = -1;
               this.var_1870 = getTimer();
               this.var_38.load(this.var_252,this.var_16);
            }
         }
      }
      ...
      {
         if(!this.var_928 && !_disposed && this.var_1159 > 0)
         {
            try
            {
               this.var_38.close();
            ...
            this.addRequestCounterToUrlRequest(this.var_252,const_1336 - this.var_1159);
            if(this.var_2596)
            {
               this.var_252.requestHeaders.push(new URLRequestHeader("pragma","no-cache"));
               this.var_252.requestHeaders.push(new URLRequestHeader("Cache-Control","no-cache"));
               this.var_2596 = false;
            }
            --this.var_1159;
            this.var_412 = 0;
      ...
      protected function loadEventHandler(param1:Event) : void
      {
         ...
         switch(param1.type)
         {
            ...
            case HTTPStatusEvent.HTTP_STATUS:
               this.var_349 = HTTPStatusEvent(param1).status;
               this.debug("Load event STATUS " + this.var_349 + " for file \"" + this.url + "\"");
               break;
            case Event.UNLOAD:
               this.debug("Load event UNLOAD for file \"" + this.url + "\"");
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_UNLOAD,this.var_349,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case ProgressEvent.PROGRESS:
               this.debug("Load event PROGRESS for file \"" + this.url + "\"");
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_PROGRESS,this.var_349,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case IOErrorEvent.IO_ERROR:
               this.debug("Load event IO ERROR for file \"" + this.url + "\"");
               if(!this.handleHttpStatus(this.var_349))
               {
                  this.var_1158 = getTimer();
         ...
         if(this.var_412 == LibraryLoader.const_1510)
         {
         ...
         if(this.var_412 == LibraryLoader.name_2)
         {
            if(this.prepareLibrary())
            {
               ...
               this.var_1158 = getTimer();
               this.removeEventListeners();
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE,this.var_349,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
      ...
      protected function prepareLibrary() : Boolean
      {
         ...
         this.var_610 = this.getDefinition(this._name) as Class;
         if(this.var_610 == null)
         {
            if(!this.retry())
            {
               this.failure("Failed to find resource class \"" + this._name + "\" from library " + this.var_252.url + "!");
            }
            return false;
         }
         try
         {
            xmlClass = this.var_610.manifest as Class;
            if(xmlClass == null)
            {
               return false;
            }
      ...
      protected function debug(param1:String) : void
      {
         ...
         if(this._debug)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DEBUG,this.var_349,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         }
      }
      ...
      protected function failure(param1:String) : void
      {
         ...
         dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR,this.var_349,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         dispatchEvent(new Event(LIBRARY_LOADER_FINALIZE));
      }
