package com.sulake.core.window.graphics.renderer
{
   ...
   public class SkinRenderer implements ISkinRenderer
   {
      ...
      public function dispose() : void
      {
         ...
         if(!this._disposed)
         {
            for(_loc1_ in this.var_506)
            {
               ISkinLayout(this.var_506[_loc1_]).dispose();
               delete this.var_506[_loc1_];
            }
            this.var_506 = null;
            this.var_409 = null;
      ...
      {
         this.var_506[param1.name] = param1;
         return param1;
      }
      ...
      public function removeLayout(param1:ISkinLayout) : ISkinLayout
      {
         ...
         if(param1 != null)
         {
            for(_loc2_ in this.var_409)
            {
               _loc3_ = uint(_loc2_ as uint);
               if(this.var_409[_loc3_] == param1)
               {
                  this.removeLayoutFromRenderState(_loc3_);
               }
            }
            delete this.var_506[param1.name];
         }
         return param1;
      }
