package com.sulake.core.runtime
{
   ...
   public class ComponentContext extends Component implements IContext
   {
      ...
      public final function loadFromFile(param1:URLRequest, param2:LoaderContext) : LibraryLoader
      {
         ...
         {
            if(this.var_251[_loc3_].url == param1.url)
            {
               return this.var_251[_loc3_].loadFromFile(param1);
            }
            _loc3_++;
         }
      ...
      protected function loadErrorHandler(param1:LibraryLoaderEvent) : void
      {
         ...
         this.removeLibraryLoader(_loc2_);
         this.error("Failed to download component resource \"" + _loc2_.url + "\"!" + "\r" + _loc3_,true,Core.const_1436);
      }
         ...
         while(i < componentList.length())
         {
            ...
            if(assetCount > 0)
            {
               ...
               while(k < assetCount)
               {
                  ...
                  {
                     this.error("Manifest for component " + componentString + " contains broken asset \"" + assetName + "\"!",true,Core.ERROR_CATEGORY_PREPARE_COMPONENT);
                  }
                  k++;
               }
            }
      ...
      protected final function announceInterfaceAvailability(param1:IID, param2:Component) : void
      {
         ...
         while(_loc6_ < _loc5_)
         {
            ...
            if(_loc4_ == null)
            {
               this.error("Interface " + param1 + " still unavailabe!",true,Core.const_1337);
            }
      ...
      override public function dispose() : void
      {
         ...
         if(!disposed)
         {
            ...
            if(this.var_251 != null)
            {
               _loc1_ = uint(this.var_251.length);
               while(_loc1_-- > 0)
               {
                  _loc2_ = this.var_251[0];
                  this.removeLibraryLoader(_loc2_);
                  _loc2_.dispose();
               }
               this.var_251 = null;
