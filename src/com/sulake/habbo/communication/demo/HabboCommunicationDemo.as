package com.sulake.habbo.communication.demo
{
   ...
   public class HabboCommunicationDemo extends Component
   {
      ...
      private var var_445:IKeyExchange;
      
      private var var_790:String;
      
      private var _view:HabboLoginDemoView;
      
      private var var_2082:PseudoRandom;
      ...
      private const const_1525:String = "fuselogin";
      ...
      {
         if(param1 && !this.var_545)
         {
            this.var_545 = param1;
            this.var_45.initConnection(HabboConnectionType.const_232);
         }
      }
      ...
      private function onRoomSessionManagerReady(param1:IID, param2:IUnknown) : void
      {
         ...
         this.var_20.events.addEventListener(RoomSessionEvent.const_92,this.onRoomSessionEnded);
         this.checkRequirements();
      }
      ...
      {
         if(this.var_45 && this.var_117 && this.var_20 && this._windowManager)
         {
      ...
      private function componentsAreRunning() : void
      {
         ...
         this.var_193 = new HabboHotelView(this._windowManager,assets,Component(context).events);
         var _loc1_:String = this.var_117.getKey("client.hotel_view.image");
         var _loc2_:String = this.var_117.getKey("image.library.url");
         if(_loc1_ != null && _loc2_ != null)
         {
            this.var_193.loadHotelViewImage(_loc2_ + _loc1_);
         }
         var _loc3_:* = this.var_117.getKey("use.sso.ticket") == "1";
         this.var_545 = this.var_117.getKey("sso.ticket",null);
         this.var_1517 = this.var_117.getKey("flash.client.url","");
         this.var_2083 = this.var_117.getKey("external.variables.txt","");
         if(this.var_193 == null)
         {
            this.var_193 = new HabboHotelView(this._windowManager,assets,Component(context).events);
         }
         if(_loc3_)
         {
            this.var_45.mode = HabboConnectionType.const_108;
            if(this.var_545)
            {
               this.var_45.initConnection(HabboConnectionType.const_232);
            }
         ...
         else if(this._windowManager != null)
         {
            ...
            if(_loc4_)
            {
               ...
               this._view.addEventListener(HabboLoginDemoView.const_427,this.onInitConnection);
            }
         }
      }
      ...
      private function onInitConnection(param1:Event = null) : void
      {
         ...
         else
         {
            ...
            {
               this.var_45.mode = HabboConnectionType.const_968;
            }
         }
         this.var_45.initConnection(HabboConnectionType.const_232);
      }
      ...
      private function onConnectionEstablished(param1:Event = null) : void
      {
         ...
         if(_loc2_ != null)
         {
            this.dispatchLoginStepEvent(HabboCommunicationEvent.const_720);
      ...
      private function onInitCrypto(param1:IMessageEvent) : void
      {
         ...
         this.var_2082 = new PseudoRandom(parseInt(_loc4_.substring(_loc4_.length - 4),16),65536);
         switch(this.var_45.mode)
         {
            case HabboConnectionType.const_108:
               _loc6_ = "";
               _loc6_ = this.var_117.getKey("url.prefix",_loc6_);
               _loc6_ = _loc6_.replace("http://","");
               _loc6_ = _loc6_.replace("https://","");
               _loc7_ = this.var_117.getKey("hotelview.banner.url","http:/\nsitename$/gamedata/banner");
               _loc7_ = _loc7_.replace("$sitename$",_loc6_);
               this.var_1218 = _loc4_;
               this.var_193.loadBannerImage(_loc7_ + "?token=" + this.var_1218,this.onHotelViewBannerLoaded);
               break;
            case HabboConnectionType.const_968:
               this.sendConnectionParameters(_loc2_);
               break;
            case HabboConnectionType.const_1428:
               this.var_790 = this.generateRandomHexString(30);
               this.var_445 = new DiffieHellman(new BigInteger(this.var_117.getKey("connection.development.prime"),16),new BigInteger(this.var_117.getKey("connection.development.generator"),16));
               this.var_445.init(this.var_790);
               _loc8_ = this.var_445.getPublicKey(10);
      ...
      private function onSecretKeyEvent(param1:IMessageEvent) : void
      {
         ...
         this.var_445.generateSharedKey(_loc4_,10);
         var _loc5_:String = this.var_445.getSharedKey(16).toUpperCase();
         var _loc6_:RC4_R27 = new RC4_R27(null,null);
         var _loc7_:IEncryption = new RC4_R27(_loc6_,this.var_2082);
      ...
      private function sendConnectionParameters(param1:IConnection) : void
      {
         ...
         connection.send(new VersionCheckMessageComposer(401,this.var_1517,this.var_2083));
         var machineId:String = "";
         try
         {
            so = SharedObject.getLocal(this.const_1525,"/");
      ...
      private function onSessionParamsEvent(param1:IMessageEvent) : void
      {
         ...
         if(this.var_545 != null)
         {
            _loc4_ = new SSOTicketMessageComposer(this.var_545);
            _loc2_.send(_loc4_);
      ...
      private function onConnectionDisconnected(param1:Event) : void
      {
         ...
         if(param1.type == Event.CLOSE && !this.var_2084)
         {
            _loc2_ = this.var_117.getKey("logout.disconnect.url");
            if(this.var_117.keyExists("facebook.user"))
            {
      ...
      private function onDisconnectReason(param1:DisconnectReasonEvent) : void
      {
         ...
         this.var_2084 = true;
         var _loc2_:String = this.var_117.getKey("logout.url");
         if(_loc2_.length > 0)
         {
      ...
      private function setOriginProperty(param1:String) : String
      {
         ...
         {
            return param1.replace("%origin%",this.var_117.getKey("flash.client.origin","popup"));
         }
         return param1;
      }
      ...
      {
         if(!this.var_20.sessionStarting)
         {
            this.showHotelView();
         }
      }
      ...
      private function onHotelViewBannerLoaded(param1:BitmapData) : void
      {
         ...
         var _loc4_:String = xor(_loc3_,this.var_1218);
         ...
         this.var_445 = new DiffieHellman(_loc10_,_loc11_);
         var _loc13_:int = 10;
         var _loc14_:* = null;
         while(_loc13_ > 0)
         {
            _loc14_ = this.generateRandomHexString(30);
            this.var_445.init(_loc14_);
            _loc15_ = this.var_445.getPublicKey(10);
            if(_loc15_.length >= 64)
            {
               _loc12_ = _loc15_;
               this.var_790 = _loc14_;
            }
