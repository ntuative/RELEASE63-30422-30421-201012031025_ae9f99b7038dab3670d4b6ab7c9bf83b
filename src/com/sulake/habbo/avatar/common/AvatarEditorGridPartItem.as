package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_650:IFigurePartSet;
      ...
      private var var_547:Rectangle;
      
      private var var_1523:int = 0;
      
      private var var_366:IAvatarRenderManager;
      
      private var var_657:Boolean;
      
      private var var_1525:BitmapData;
      
      private const const_1798:int = 13421772;
      
      private const const_1797:int = 16777215;
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         this._window = param1;
         this._colors = param4;
         this.var_1524 = param5;
         if(param3 == null)
         {
            this.var_622 = new BitmapData(1,1,true,16777215);
         }
         if(param3 != null)
         {
            _loc7_ = param3.parts;
            ...
            {
               this.var_1523 = Math.max(this.var_1523,_loc6_.colorLayerIndex);
            }
         }
         this.var_366 = this.var_62.controller.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_622 != null && !this.var_1524)
            {
               _loc3_ = this.var_622;
            }
            else
         ...
         if(_loc2_)
         {
            if(this.var_650)
            {
               switch(this.var_650.clubLevel)
               {
                  case HabboClubLevelEnum.const_49:
                     _loc2_.visible = false;
                     break;
                  case HabboClubLevelEnum.const_60:
                     _loc2_.style = HabboIconType.const_803;
                     break;
                  case HabboClubLevelEnum.const_53:
                     _loc2_.style = HabboIconType.const_778;
               }
            }
            else
            {
         ...
         if(this.isSelected)
         {
            this._window.color = this.const_1798;
         }
         else
         {
            this._window.color = this.const_1797;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:AvatarFigureContainer = this.var_366.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_366.isFigureReady(_loc1_))
         {
            this.var_366.downloadFigure(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_309[_loc6_] + "_" + FigureData.const_132;
               _loc4_ = this.var_366.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_547 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_1525)
               {
                  ...
                  this.var_1525 = _loc9_.content as BitmapData;
               }
         ...
         _loc1_ = new BitmapData(this.var_547.width,this.var_547.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_547.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_547.y;
               if(this.var_1524 && _loc2_.colorLayerIndex > 0)
               {
