package com.sulake.habbo.widget.roomqueue
{
   ...
   public class RoomQueueWidget extends RoomWidgetBase
   {
      ...
      private function onQueueStatus(param1:RoomWidgetRoomQueueUpdateEvent) : void
      {
         ...
         if(param1.isActive)
         {
            this.var_1441 = param1.type;
            this.var_1178 = false;
            this.var_941 = param1.position;
         }
         else
         {
            this.var_1178 = true;
         }
         this.var_1963 = param1.hasHabboClub;
         this.var_1442 = param1.isClubQueue;
         localizations.registerParameter("room.queue.position","position",this.var_941.toString());
         localizations.registerParameter("room.queue.position.hc","position",this.var_941.toString());
         localizations.registerParameter("room.queue.spectator.position","position",this.var_941.toString());
         localizations.registerParameter("room.queue.spectator.position.hc","position",this.var_941.toString());
         this.showInterface();
      }
         ...
         if(_loc1_ != null && _loc2_ != null && _loc3_ != null)
         {
            switch(this.var_1441)
            {
               case RoomWidgetRoomQueueUpdateEvent.const_341:
                  _loc1_.caption = !!this.var_1442 ? "${room.queue.position.hc}" : "${room.queue.position}";
                  _loc2_.caption = "${room.queue.spectatormode}";
                  _loc3_.visible = this.var_1178;
                  break;
               case RoomWidgetRoomQueueUpdateEvent.const_447:
                  _loc1_.caption = !!this.var_1442 ? "${room.queue.spectator.position.hc}" : "${room.queue.spectator.position}";
                  _loc2_.caption = "${room.queue.back}";
                  _loc3_.visible = false;
            }
            _loc2_.visible = this.var_1178;
         }
         var _loc4_:IWindowContainer = this._window.findChildByName("club_container") as IWindowContainer;
         if(_loc4_ != null)
         {
            _loc4_.visible = !this.var_1963;
         }
      ...
      private function exitQueue(param1:WindowMouseEvent) : void
      {
         ...
         var _loc2_:RoomWidgetRoomQueueMessage = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.const_602);
      ...
      {
         messageListener.processWidgetMessage(new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.const_589));
      }
      ...
      private function changeQueue(param1:WindowMouseEvent) : void
      {
         ...
         if(this.var_1441 == RoomWidgetRoomQueueUpdateEvent.const_341)
         {
