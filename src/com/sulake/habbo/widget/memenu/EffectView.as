package com.sulake.habbo.widget.memenu
{
   ...
   public class EffectView
   {
      ...
      private function onUpdate(param1:Event = null) : void
      {
         ...
         {
            if(this.var_166.isActive)
            {
               _loc2_ = this.var_166.secondsLeft / Number(this.var_166.duration);
               this.var_873.width = _loc2_ * this.var_421;
            }
            else
            {
               this.var_873.width = 0;
               this.var_181.stop();
            }
            this.setTimeLeft();
         }
      ...
      private function setTimeLeft() : void
      {
         ...
         {
            this.var_871.caption = "${widgets.memenu.effects.active.timeleft}";
            _loc1_ = this.var_871.text;
            if(this.var_166.isActive)
            {
               _loc2_ = this.var_166.secondsLeft;
               ...
               this.var_871.text = _loc1_;
            }
            else
            {
               this.var_871.caption = "${widgets.memenu.effects.activate}";
            }
         }
      }
      ...
      private function createWindow(param1:String) : void
      {
         ...
         var _loc7_:* = null;
         var _loc3_:String = "";
         if(this.var_166.isInUse)
         {
            _loc3_ = "memenu_effect_selected";
         }
         else if(this.var_166.isActive)
         {
         ...
         else
         {
            ...
            this.var_872 = this._window.findChildByName("effect_hilite");
            this.var_872.visible = false;
         }
         this.var_871 = this._window.findChildByName("time_left") as ITextWindow;
         this.setTimeLeft();
         this.var_873 = this._window.findChildByName("loader_bar");
         if(this.var_873 != null)
         {
            this.var_421 = this.var_873.width;
            this.var_181.start();
            this.onUpdate();
         }
         if(this.var_166.icon)
         {
            this.setElementBitmap("effect_icon",this.var_166.icon);
         }
      }
