package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class ColourGridCatalogWidget extends CatalogWidget implements ICatalogWidget
   {
      ...
      private function select(param1:IWindowContainer) : void
      {
         ...
         {
            _loc2_ = this.var_312.getChildByName("chosen");
            if(_loc2_ != null)
            {
      ...
      private function populateColourGrid() : void
      {
         ...
         var _loc13_:* = null;
         var _loc14_:* = 0;
         Logger.log("[ColourGridCatalogWidget] Display colors: " + [this.var_1373.length,this._colourGrid.numGridItems]);
         this._colourGrid.destroyGridItems();
         this.var_312 = null;
         for each(_loc1_ in this.var_1373)
         {
            ...
            _loc2_.width = this.var_737.width;
            _loc2_.height = this.var_737.height;
            this._colourGrid.addGridItem(_loc2_);
            _loc3_ = _loc2_.findChildByTag("BG_BORDER") as IBitmapWrapperWindow;
            if(_loc3_ != null)
            {
               _loc3_.bitmap = new BitmapData(this.var_737.width,this.var_737.height,true,0);
               _loc3_.bitmap.copyPixels(this.var_737,this.var_737.rect,new Point(0,0));
            }
            _loc4_ = _loc2_.findChildByTag("COLOR_IMAGE") as IBitmapWrapperWindow;
            if(_loc4_ != null)
            {
               _loc4_.bitmap = new BitmapData(this.var_1120.width,this.var_1120.height,true,0);
               _loc6_ = 255;
               _loc7_ = 255;
               _loc8_ = 255;
               if(_loc1_ > 0)
            ...
            if(_loc5_ != null)
            {
               _loc5_.bitmap = new BitmapData(this.var_882.width,this.var_882.height,true,16777215);
               _loc5_.bitmap.copyPixels(this.var_882,this.var_882.rect,new Point(0,0),null,null,true);
