package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IPetImageListener
   {
      ...
      override public function init() : void
      {
         ...
         this.var_885 = this.getPetTypeIndexFromProduct(_loc5_.localizationId);
         this.var_336 = [];
         this.var_606 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc9_ = _loc8_.avatarRenderManager.petDataManager.getPetData(this.var_885,this);
               if(_loc9_)
               {
                  this.var_606 = _loc9_.sellableBreeds;
                  this.var_336 = _loc9_.colors;
         ...
         var _loc6_:int = 0;
         while(_loc6_ < this.var_606.length)
         {
            _loc10_ = this.var_606[_loc6_] as IBreed;
            _loc3_.push(_loc4_.getKey(this.getRaceLocalizationKey(this.var_885,_loc10_.id),this.getRaceLocalizationKey(this.var_885,_loc10_.id)));
            _loc6_++;
         }
         this._offers.add(this.var_885,_loc5_);
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         this.var_489 = param1.index;
         if(this.var_489 < 0 || this.var_489 > this.var_336.length)
         {
            this.var_489 = 0;
         }
         this.updateImage();
      ...
      private function updateImage() : void
      {
         ...
         if(this.var_489 >= 0 && this.var_489 < this.var_336.length)
         {
            _loc7_ = this.var_336[this.var_489] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = this.var_606[this.var_1783] as IBreed;
         if(!_loc3_)
         {
            return;
         }
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(this.var_885,_loc3_.id,_loc2_,AvatarScaleType.const_48,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_273,AvatarAction.const_641);
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(this.var_489 >= this.var_336.length)
         {
            return "";
         }
         var petColor:IPetColor = this.var_336[this.var_489] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:IBreed = this.var_606[this.var_1783] as IBreed;
